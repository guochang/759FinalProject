...
void check(int *puzzle_sol){
	...
	float checksum[9] = { 45, 45, 45, 45, 45, 45, 45, 45, 45 };
	...
	if (zero == 0){
		...
		printf("\nThe output 9x9 sudoku result is correct.\n");
	}
	else{
	...
	stat = cublasGetVector(N, sizeof(float), d_checksum, 1, y, 1);
	for (i = 0; i < N; i++){
		printf("%f\n", y[i]);
	}*/
		...
		for(empty_index=0; (empty_index < num_empty[index]) && (empty_index >= 0); ){
			...
			if(r_flag == 1){
				...
				if(c_flag == 1){
				    ...
				    for(int r = 0; r < n; r++){
					   for(int c = 0; c < n; c++){
						   if(( (r_d*n+r)*N + c_d*n + c) != pos){
							   val = new_array[index*N*N + (r_d*n+r)*N + c_d*n + c ];
